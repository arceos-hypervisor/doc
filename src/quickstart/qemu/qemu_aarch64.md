...
建议从源码编译安装 9.2.2 版本，见注意事项

## ArceOS

首先，获取 ArceOS 主线代码，然后执行获取 `helloworld_aarch64-qemu-virt.bin`

```bash
git clone https://github.com/arceos-org/arceos.git
cd arceos
make PLATFORM=aarch64-qemu-virt SMP=1 A=examples/helloworld
```

`helloworld_aarch64-qemu-virt.bin` 位于 `/path/to/arceos/examples/helloworld`

### 从文件系统加载运行

以下操作均在 `/path/to/axvisor/` 下进行
...
首先，获取 Linux 主线代码，再获取 `Image`（位于 `/path/to/linux-6.2.0/arch/arm64/boot/`）

```bash
git clone git@github.com:arceos-hypervisor/linux-6.2.0.git
cd linux-6.2.0
make ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- defconfig
make ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- -j$(nproc)
```

然后，编译 Linux 客户机需要使用的设备树文件 `linux-qemu.dtb`

```bash
cd axvisor
dtc -I dts -O dtb -o linux-qemu.dtb configs/vms/linux-qemu.dts
```
...
3. 执行 `make ARCH=aarch64 VM_CONFIGS=configs/vms/linux-qemu-aarch64.toml LOG=debug BUS=mmio NET=y FEATURES=page-alloc-64g MEM=8g run` 构建 AxVisor，并在 QEMU 中启动。
   ...
   [    0.000000] Linux version 6.9.0-rc5-00159-gc942a0cd3603 (jhwang@dell) (aarch64-linux-gnu-gcc (Ubuntu 13.3.0-6ubuntu2~24.04) 13.3.0, GNU ld (GNU Binutils for Ubuntu) 2.42) #1 SMP PREEMPT Sat May 24 19:04:40 CST 2025
...
2. 修改对应的 `configs/vms/linux-qemu-aarch64.toml-vm2` 和 `configs/vms/arceos-aarch64.toml` 中的配置项
   - `image_location="memory"` 配置项
   - `kernel_path` 指定内核镜像在工作空间中的相对/绝对路径
   - `entry_point` 指出内核镜像的入口地址
   - `kernel_load_addr` 指出内核镜像的加载地址
